...
const SPREADSHEET_ID = "1QF9QcRZkac50kZjVsMjVcESozR8m2M2yX4TxQ4UvLCk";
...
async function fetchMonitoringData() {
  ...
  const resp = await sheets.spreadsheets.values.get({
    ...
    range: "Sheet1!A:H",  // ✅ make sure "Sheet1" matches your actual tab name
  });
  ...
  rows.forEach((row) => {
    // detect header row (if it contains "Item ID" and "Item Name")
    if (row.includes("Item ID") && row.includes("Item Name")) {
