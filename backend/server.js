...
const CreativeRequest = require("./models/CreativeRequest") // Creative request model
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(400)
        .json({ message: "Invalid username or account not approved" })
    ...
    if (status === "For Approval") {
      ...
      if (validationErrors.length > 0) {
        return res
          ...
          .json({
            message:
              "Contract must be fully filled before sending for approval:\n\n" +
...
app.put("/contracts/:id", async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id))
      return res.status(400).json({ message: "Invalid contract id" })
    const { page1 = {}, page2 = {}, page3 = {}, status, rejectionReason } =
      req.body
    const contract = await Contract.findById(id)
    if (!contract) return res.status(404).json({ message: "Not found" })
    if (!["Draft", "Rejected"].includes(contract.status))
      return res
        .status(400)
        .json({ message: "Only Draft or Rejected contracts can be edited" })
    // Update the fields
...
app.put("/contracts/:id/approve", async (req, res) => {
  try {
    ...
    const contract = await Contract.findById(id)
    if (!contract) return res.status(404).json({ message: "Not found" })
    if (contract.status !== "For Approval")
      return res
        .status(400)
        .json({
          message: "Only contracts with 'For Approval' status can be approved",
        })
...
app.put("/contracts/:id/reject", async (req, res) => {
  try {
    ...
    if (contract.status.trim().toLowerCase() !== "for approval")
      return res
        .status(400)
        .json({
          message: "Only contracts with 'For Approval' status can be rejected",
        })
...
app.put("/contracts/:id/accounting-approve", async (req, res) => {
  try {
    ...
    const contract = await Contract.findById(id)
    if (!contract) return res.status(404).json({ message: "Not found" })
    if (contract.status !== "For Accounting Review")
      return res
        .status(400)
        .json({
          message:
            "Only contracts with 'For Accounting Review' status can be approved by Accounting",
        })
  ...
  try {
    ...
    if (contract.status !== "For Accounting Review")
      return res
        ...
        .json({
          message:
            "Only contracts with 'For Accounting Review' status can be rejected by Accounting",
        })
...
const validateContractFullyFilled = (contract) => {
  ...
  Object.entries(contract.page1 || {}).forEach(([key, value]) => {
    if (typeof value === "string" && !value.trim()) {
      errors.push(
        `Page 1 - ${key.replace(/([A-Z])/g, " $1").toLowerCase()} is required`
      )
    }
  })
  ...
  Object.entries(contract.page2 || {}).forEach(([key, value]) => {
    if (typeof value === "string" && !value.trim()) {
      errors.push(
        `Page 2 - ${key.replace(/([A-Z])/g, " $1").toLowerCase()} is required`
      )
    }
  })
  ...
  Object.entries(contract.page3 || {}).forEach(([key, value]) => {
    if (typeof value === "string" && !value.trim()) {
      errors.push(
        `Page 3 - ${key.replace(/([A-Z])/g, " $1").toLowerCase()} is required`
      )
    }
  })
  return errors
...
// GET /creativeRequests - Fetch all creative requests (newest first)
...
// POST /creativeRequests - Create a new creative request
app.post("/creativeRequests", async (req, res) => {
  try {
    let {
      ...
      materials,
      notes,
      materialsNeeded, // support legacy field if sent
    } = req.body;
    // If materials is not provided and materialsNeeded is sent, create a materials array.
    if (!materials && materialsNeeded) {
      materials = [{ name: materialsNeeded }];
    }
...
// PUT /creativeRequests/:id - Update an existing creative request
app.put("/creativeRequests/:id", async (req, res) => {
  try {
    const { id } = req.params;
    let { requestName, designer, dueDate, status, materials, notes, materialsNeeded } = req.body;
    ...
    if (materials || materialsNeeded) {
      creativeRequest.materials = materials
        ? (Array.isArray(materials) ? materials : [{ name: materials }])
        : [{ name: materialsNeeded }];
    }
    if (notes !== undefined) creativeRequest.notes = notes;
    await creativeRequest.save();
...
// DELETE /creativeRequests/:id - Delete a creative request
