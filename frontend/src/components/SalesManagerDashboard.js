...
function SalesManagerDashboard({ onLogout, user }) {
  ...
  // Approve contract and send to accounting
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setRejectReason("");
      setSelectedContract(null);
      alert("Contract has been rejected");
    } catch (err) {
      console.error(err);
  ...
  const renderContractsTable = () => (
    <div className="contracts-table-container">
      ...
      <div className="status-tabs">
        {["All", "For Approval", "For Accounting Review", "Active", "Completed", "Rejected"].map(status => (
          <button
            key={status}
          ...
          {selectedContract?.status === "For Approval" && (
            <div className="approval-actions">
              <button 
                ...
                onClick={() => handleApprove(selectedContract._id)}
              >
              ...
              <button 
                ...
                onClick={() => {
                  setReviewingContract({
                    ...
                    name: selectedContract.page1?.contractName || selectedContract.page1?.occasion || ""
                  });
        ...
        <div className="modal-body">
          {reviewingContract && (
            <div className="reject-info">
              <p><strong>Contract:</strong> {reviewingContract.contractNumber}</p>
              <p><strong>Client:</strong> {reviewingContract.client}</p>
              <p><strong>Event:</strong> {reviewingContract.name}</p>
            </div>
          )}
          <div className="form-group">
