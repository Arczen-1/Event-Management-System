...
function LinenDashboard({ onLogout }) {
  ...
  const [currentInventoryDepartment, setCurrentInventoryDepartment] = useState("linen");
      ...
      if (res.ok) {
        ...
        setContracts(
          (data.contracts || [])
            ...
            .map(c => {
              const existingChecklist = checklistsData.find(cl => cl.contractId === c._id && cl.department === 'linen');
              return {
                id: c._id,
  ...
  const fetchInventory = async (department = "linen") => {
    try {
  ...
  const openChecklistModal = async (contract, editMode = false, checklistId = null) => {
    ...
    if (editMode && checklistId) {
      ...
      const existingChecklist = submittedChecklists.find(cl => 
        cl.contractId === contract._id && cl.department === 'linen'
      );
      
      if (existingChecklist) {
        setMessage("A linen checklist already exists for this contract. Use the Edit button.");
        return;
      }
  ...
  const generateChecklistItems = (contract) => {
    ...
    // Linen department items - table linens, napkins, chair covers, etc.
    const linenFields = ['linen', 'napkin', 'chairCover', 'tableRunner', 'skirting'];
    
    linenFields.forEach(field => {
  ...
  const submitChecklist = async () => {
    try {
      ...
      if (isEditingChecklist && editingChecklistId) {
        ...
        const existingChecklist = submittedChecklists.find(cl => 
          cl.contractId === selectedContract._id && cl.department === 'linen'
        );
        
        if (existingChecklist) {
          setMessage("A linen checklist already exists for this contract. Please use the Edit button.");
          return;
        }
      }
      ...
      if (res.ok) {
        ...
        setMessage(`Linen checklist ${isEditingChecklist ? 'updated' : 'submitted'} successfully!`);
  ...
  const downloadFabricationReport = () => {
    ...
    a.href = url;
    a.download = `linen-fabrication-report-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  // ------------------- Inventory CRUD -------------------
  const openInventoryModal = (mode, data = {}, id = null, department = "linen") => {
  ...
  const deleteInventoryItem = async (id, department = "linen") => {
  ...
  const renderInventoryTable = () => {
    return (
      <div className="contracts-table-container">
        <div className="table-header">
          <h3>Linen Inventory Management</h3>
          <div className="table-actions">
            <button className="btn-add" onClick={() => openInventoryModal("add", {}, null, "linen")}>
              Add New Item
            </button>
          </div>
        </div>
        ...
        {inventoryData.length === 0 ? (
          <div className="no-data">
            <p>No inventory data found for linen department.</p>
            <button 
              className="btn-primary" 
              onClick={() => fetchInventory("linen")}
            >
              Refresh Data
            </button>
          </div>
                  ...
                  <tr key={item._id || idx}>
                    ...
                    <td>
                      <div className="action-buttons">
                        <button 
                          className="btn-edit" 
                          onClick={() => openInventoryModal("edit", item, item._id, "linen")}
                        >
                          Edit
                        </button>
                        <button 
                          className="btn-delete" 
                          onClick={() => deleteInventoryItem(item._id, "linen")}
                        >
                          Delete
                        </button>
                      </div>
    ...
    checklistModalOpen && selectedContract && (
      <div className="modal-overlay" onClick={() => setChecklistModalOpen(false)}>
        <div className="modal-content large-modal" onClick={e => e.stopPropagation()}>
          <div className="modal-header">
            <h3>
              {isEditingChecklist ? 'Edit' : 'Create'} Linen Checklist - {selectedContract.contractNumber}
            ...
            <div className="checklist-container">
              <div className="checklist-header">
                ...
                <div className="department-notice">
                  <strong>Linen Department Only</strong>
                  <p>This checklist is for Linen items only: Table Linens, Napkins, Chair Covers, Table Runners, and Skirting.</p>
                  <p>Contract will be marked as Completed when all departments (Creative, Linen, Warehouse) submit their checklists.</p>
                </div>
                {isEditingChecklist && (
                  <div className="edit-notice">
                    <span>⚠ Editing existing Linen checklist.</span>
                  </div>
                )}
              </div>

              <div className="checklist-items">
                <h5>Linen Items Checklist</h5>
                {checklistItems.length === 0 ? (
                  <p className="no-items">No linen items found for this event.</p>
                ) : (
                      ...
                      <div key={item.id} className="checklist-item">
                        <div className="checklist-item-content">
                          <div className="item-main-info">
                            ...
                            <span className="item-department-badge">Linen</span>
                          </div>
          ...
          <div className="modal-actions">
            ...
            >
              {isEditingChecklist ? 'Update Linen Checklist' : 'Submit Linen Checklist'}
            </button>
            <button 
              className="btn-secondary" 
              onClick={() => {
